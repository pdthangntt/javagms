...
LEFT JOIN
 (select * from (SELECT kq2.*
   ...
   AND re.stage_id = kq2.stage_id
   -- AND kq2.result = '4'
   AND DATE_FORMAT(re.treatment_time, '%Y-%m-%d') <= DATE_FORMAT(kq2.test_time, '%Y-%m-%d')) AS final_kq2 GROUP BY final_kq2.arv_id) as main_final_kq2  ON main_final_kq2.arv_id = final_rev.arv_id
LEFT JOIN opc_visit vi
   ...
   WHERE (@keywords IS NULL OR @keywords = '' OR tbl.code LIKE CONCAT('%',@keywords, '%') OR tbl.fullname LIKE CONCAT('%',@keywords, '%') 
...
ORDER BY code DESC LIMIT @offsetNumber, @size