SELECT
    ...
    m.district,
    m.ward,
    CASE WHEN m.objectGroupID = '1' THEN COUNT(m.id) ELSE 0 END                            AS ntmt,
    CASE WHEN m.objectGroupID = '2' THEN COUNT(m.id) ELSE 0 END                            AS md,
    CASE WHEN m.objectGroupID = '3' THEN COUNT(m.id) ELSE 0 END                            AS msm,
    CASE WHEN m.objectGroupID = '4' THEN COUNT(m.id) ELSE 0 END                            AS vcbtbc,
    CASE WHEN (m.objectGroupID = '5' 
            OR m.objectGroupID = '5.1' 
            OR m.objectGroupID = '5.2') THEN COUNT(m.id) ELSE 0 END                        AS pnmt,
    CASE WHEN m.objectGroupID = '6' THEN COUNT(m.id) ELSE 0 END                            AS lao,
    CASE WHEN m.objectGroupID = '100' THEN COUNT(m.id) ELSE 0 END                          AS khongthongtin,
    CASE WHEN m.objectGroupID = '25' THEN COUNT(m.id) ELSE 0 END                           AS hoalieu,
    CASE WHEN m.objectGroupID = '19' THEN COUNT(m.id) ELSE 0 END                           AS phamnnhan,
    CASE WHEN ( m.objectGroupID = '29' 
            OR m.objectGroupID = '501' ) THEN COUNT(m.id) ELSE 0 END                       AS mecon,
    CASE WHEN m.objectGroupID = '12' THEN COUNT(m.id) ELSE 0 END                           AS nghiavu,
    CASE WHEN m.objectGroupID <> '1' 
          AND m.objectGroupID <> '2' 
          AND m.objectGroupID <> '3' 
          AND m.objectGroupID <> '4' 
          AND m.objectGroupID <> '5' 
          AND m.objectGroupID <> '5.1' 
          AND m.objectGroupID <> '5.2' 
          AND m.objectGroupID <> '6' 
          AND m.objectGroupID <> '100' 
          AND m.objectGroupID <> '25' 
          AND m.objectGroupID <> '19' 
          AND m.objectGroupID <> '29' 
          AND m.objectGroupID <> '501' 
          AND m.objectGroupID <> '12' 
          OR m.objectGroupID is null THEN COUNT(m.id) ELSE 0 END                          AS khac
FROM
(
SELECT 
    e.id,
...
GROUP BY m.province, m.district, m.ward, m.objectGroupID
