...
main.province,
coalesce(SUM(main.blood), 0) as blood,
coalesce(SUM(main.sexuality), 0) as sexuality,
coalesce(SUM(main.momtochild), 0) as momtochild,
coalesce(SUM(main.unclear), 0) as unclear,
coalesce(SUM(main.noinformation), 0) as noinformation
FROM (	SELECT 	m.permanent_province_id province, 
                CASE WHEN m.modeOfTransmissionID IN (@blood) THEN COUNT(m.id) ELSE 0 END                            AS blood,
                CASE WHEN m.modeOfTransmissionID IN (@sexuality) THEN COUNT(m.id) ELSE 0 END                        AS sexuality,
                CASE WHEN m.modeOfTransmissionID = @momtochild THEN COUNT(m.id) ELSE 0 END                       AS momtochild,
                CASE WHEN m.modeOfTransmissionID IN (@unclear) THEN COUNT(m.id) ELSE 0 END                          AS unclear,
                CASE WHEN (m.modeOfTransmissionID is null OR m.modeOfTransmissionID = '') THEN COUNT(m.id) ELSE 0 END  AS noinformation
	FROM (
		SELECT 	e.id,
                        e.permanent_province_id,
                        e.detect_province_id,
                        e.province_id,
                e.MODE_OF_TRANSMISSION_ID as modeOfTransmissionID
	...
	) as m GROUP BY m.permanent_province_id, m.modeOfTransmissionID
...
FROM (	SELECT 	m.detect_province_id province, 
	...
	) as m GROUP BY m.detect_province_id, m.modeOfTransmissionID
) as main GROUP BY main.province
...
FROM (	SELECT 	m.province_id province, 
	...
	) as m GROUP BY m.province_id, m.modeOfTransmissionID
) as main GROUP BY main.province
