SELECT
    ...
    m.district,
    m.ward,
    CASE WHEN m.insurance = '1' THEN COUNT(m.id) ELSE 0 END AS insurance_yes,
    CASE WHEN m.insurance = '0' THEN COUNT(m.id) ELSE 0 END AS insurance_no,
    CASE WHEN m.insurance = '3' OR m.insurance IS NULL OR m.insurance = '' THEN COUNT(m.id) ELSE 0 END AS other
FROM
(
SELECT 
    e.id,
    ...
    END AS ward,
    e.has_health_insurance AS insurance
FROM pac_patient_info as e
WHERE 
    e.is_remove = false
    AND (e.gender_id = @genderID OR @genderID IS NULL OR @genderID = '')
    AND (
        (
...
GROUP BY m.province, m.district, m.ward, m.insurance
