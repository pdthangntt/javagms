...
(
    ...
    WHERE 
        ...
        AND (@dateFilter = 'ngayxn' AND @from1 IS NOT NULL AND (YEAR(e.confirm_time) - e.year_of_birth) >= @from1 AND (YEAR(e.confirm_time) - e.year_of_birth) <= @to1 OR @dateFilter <> 'ngayxn')
        AND (@dateFilter = 'hientai' AND @from1 IS NOT NULL AND (YEAR(CURDATE()) - e.year_of_birth) >= @from1 AND (YEAR(CURDATE()) - e.year_of_birth) <= @to1 OR @dateFilter <> 'hientai')
