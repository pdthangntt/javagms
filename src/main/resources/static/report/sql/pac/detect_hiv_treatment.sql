SELECT 
        ...
        m.district, 
        m.ward, 
        CASE WHEN m.statusOfTreatmentID = '3' THEN COUNT(m.id) ELSE 0 END AS trongtinh ,
        CASE WHEN m.statusOfTreatmentID = '9' THEN COUNT(m.id) ELSE 0 END AS ngoaitinh,
        CASE WHEN m.statusOfTreatmentID = '1' THEN COUNT(m.id) ELSE 0 END AS chuadieutri,
        CASE WHEN m.statusOfTreatmentID = '2' THEN COUNT(m.id) ELSE 0 END AS chodieutri,
        CASE WHEN m.statusOfTreatmentID = '4' THEN COUNT(m.id) ELSE 0 END AS bodieutri,
        CASE WHEN m.statusOfTreatmentID = '6' THEN COUNT(m.id) ELSE 0 END AS matdau,
        CASE WHEN m.statusOfTreatmentID = '7' THEN COUNT(m.id) ELSE 0 END AS tuvong,
        CASE WHEN m.statusOfTreatmentID <> '3'
              AND m.statusOfTreatmentID <> '9'
              AND m.statusOfTreatmentID <> '1'
              AND m.statusOfTreatmentID <> '2'
              AND m.statusOfTreatmentID <> '4'
              AND m.statusOfTreatmentID <> '6'
              AND m.statusOfTreatmentID <> '7'
               OR m.statusOfTreatmentID IS NULL OR m.statusOfTreatmentID = '' OR m.statusOfTreatmentID = '5' THEN COUNT(m.id) ELSE 0 END AS khongthongtin
    FROM 
    (SELECT 
        e.id,
    ...
    GROUP BY m.province, m.district, m.ward, m.statusOfTreatmentID
