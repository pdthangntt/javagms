SELECT 
    ...
    m.district, 
    m.ward, 
    CASE WHEN m.mode_of_transmission_id IN ('1', '5' ,'6') THEN COUNT(m.id) ELSE 0 END AS column1,
    CASE WHEN m.mode_of_transmission_id IN ('2', '8', '9') THEN COUNT(m.id) ELSE 0 END AS column2,
    CASE WHEN m.mode_of_transmission_id IN ('3') THEN COUNT(m.id) ELSE 0 END AS column3,
    CASE WHEN m.mode_of_transmission_id IN ('4', '7') THEN COUNT(m.id) ELSE 0 END AS column4,
    CASE WHEN (m.mode_of_transmission_id = '' OR m.mode_of_transmission_id IS NULL) THEN COUNT(m.id) ELSE 0 END AS column5
FROM 
(SELECT 
        e.id,
        ...
        END AS ward,
        e.mode_of_transmission_id AS mode_of_transmission_id
...
GROUP BY m.province, m.district, m.ward, m.mode_of_transmission_id