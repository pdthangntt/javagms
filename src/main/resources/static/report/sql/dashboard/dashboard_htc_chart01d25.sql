...
FROM (
    SELECT 
        case 
            when (e.virus_load_date IS NOT NULL ) then DATE_FORMAT(e.virus_load_date, '%c/%Y')
        END as d, 
        case 
            when (e.virus_load IS NOT NULL AND e.virus_load = '2') then 1 else 0 
        END as nhohon6, 
        0 as lonhon12,
        0 as total 
    ...
    SELECT 
        ...
        0 as nhohon6,
        case 
            when (e.virus_load IS NOT NULL AND e.virus_load = '3') then 1 else 0 
        END as lonhon12,
        0 as total 
    FROM htc_confirm as e 
    WHERE e.is_remove = 0
