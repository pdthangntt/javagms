SELECT 
    revision.site_id,
    COALESCE(COUNT(revision.arv_id), 0) 
FROM 
    (
        SELECT r.*
           ...
           FROM
             (SELECT re.*
              ...
              WHERE (re.site_id IN (@site_id))
                AND re.is_remove = 0
                AND DATE_FORMAT(re.created_at, '%Y-%m-%d') <= @lastDate ORDER BY created_at DESC, id DESC) AS m
           GROUP BY m.arv_id) AS max_time ON r.id = max_time.id
    ...
    ) as revision GROUP BY revision.site_id
