...
FROM (
        ...
        FROM (
            ...
            ON main_log.arv_id_log = main_visit.arv_id AND main_log.stage_id_log = main_visit.stage_id AND main_stage.stage_id_main = main_log.stage_id_log
            WHERE main_log.is_remove = 0 AND main_stage.is_remove_stage = 0
            -- Điều kiện
            AND DATE_FORMAT(main_log.treatment_time , '%Y-%m-%d') <= @to_date
            AND main_log.tranfer_from_time is null
            AND main_log.end_time is null
