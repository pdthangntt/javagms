SELECT 
    ...
    sum(main.nl) as nl
FROM (SELECT case 
            when (sub_main.test_time IS NOT NULL ) then CONCAT(QUARTER(sub_main.test_time), '/', YEAR(sub_main.test_time))
        END as d, 
     ...
     FROM (
            ...
            WHERE v.test_time is not null 
                AND s.is_remove = 0  
                AND DATE_FORMAT(v.test_time, '%Y-%m-%d') BETWEEN MAKEDATE(YEAR(@from_date), 1) + INTERVAL QUARTER(@from_date) QUARTER - INTERVAL    1 QUARTER 
                ...
                AND v.id IN (
                    ...
                    FROM (select * from (SELECT *
                            FROM opc_arv_revision as logmax 
                                            WHERE DATE_FORMAT(logmax.created_at, '%Y-%m-%d') <= (MAKEDATE(YEAR(@from_date), 1) + INTERVAL QUARTER(@from_date) QUARTER - INTERVAL 1 DAY)  AND logmax.site_id IN (@site_id)
