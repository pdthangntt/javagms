...
FROM (
    SELECT 
        ...
        END as d, 
        case 
            when (e.results_patien_time IS NOT NULL AND e.confirm_results_id ='2') then 1 else 0 
        END as nhanketqua, 
        0 as total 
    FROM htc_visit as e 
    WHERE e.is_remove = 0 
    ...
    SELECT 
        case 
            when (e.confirm_time IS NOT NULL) then DATE_FORMAT(e.confirm_time, '%c/%Y')
        END as d, 
        0 as nhanketqua, 
        case when (e.confirm_time IS NOT NULL  AND e.confirm_results_id ='2') then 1 else 0 end as total 
    FROM htc_visit as e 
    WHERE e.is_remove = 0 
        AND ( e.site_id IN (@site_id) OR coalesce(@site_id, null) IS NULL) 
        AND ( e.service_id IN (@services) OR coalesce(@services, null) IS NULL) 
