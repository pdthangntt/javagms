   ...
   FROM
     ...
     SELECT t.id,
      ...
      FROM
        ...
        INNER JOIN (SELECT v.arv_id,
						   v.inh_to_time,
						   v.inh_end_cause
					FROM pqm_arv_test AS v
					WHERE 1=1
                                        AND v.inh_to_time is not null
					  AND DATE_FORMAT(v.inh_to_time, '%Y-%m-%d') BETWEEN @from_date AND @to_date
					  ) as visit ON visit.arv_id = stage.arv_id
	) AS t
      ...
      WHERE 1=1
          ...
          AND ( t.end_time IS NULL OR ( t.end_time IS NOT NULL AND DATE_FORMAT(t.end_time, '%Y-%m-%d')  > @to_date )) 
          AND t.inh_to_time >= t.treatment_time
          AND (t.end_time is null OR (t.end_time is not null AND t.inh_to_time <= t.end_time))
          
          
        
     ) as a_main group by a_main.arv_id
