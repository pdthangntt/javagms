  ...
  (SELECT v_main.*,
   ...
   FROM
     (SELECT t.id,
             ...
             t.object_group_id,
             t.dob,
             DATE_FORMAT(t.treatment_time, '%Y') - DATE_FORMAT(t.dob, '%Y') AS age,
             t.REGISTRATION_TYPE
      FROM
        (SELECT stage.*,
                arv.dob,
           ...
           (SELECT *
            FROM
              (SELECT s.*
               ...
               ORDER BY s.treatment_time ASC) AS st
            GROUP BY st.arv_id) AS stage
         INNER JOIN pqm_arv AS arv ON stage.arv_id = arv.id) AS t
      WHERE 1=1
        AND t.site_id IN (@site_id)
        AND t.treatment_time IS NOT NULL
        AND t.REGISTRATION_TYPE = '1'
        AND DATE_FORMAT(t.treatment_time, '%Y-%m-%d') BETWEEN @from_date AND @to_date ) AS v_main) AS v_group
