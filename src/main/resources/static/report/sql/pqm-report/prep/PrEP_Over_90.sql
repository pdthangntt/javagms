...
SELECT t.id, 
        ...
        FROM `pqm_prep` as t
        INNER JOIN (SELECT v.prep_id, v.examination_time FROM pqm_prep_visit v WHERE v.examination_time is not null 
        ...
        WHERE 1 = 1
             ...
             AND DATEDIFF(@to_date, DATE_FORMAT(t.start_time, '%Y-%m-%d')) >= 90 
) as d_table
GROUP BY d_table.id  
