...
from (select 
    ...
    from (SELECT t.id, 
            ...
            t.object_group_id, 
            t.year_of_birth, 
            DATE_FORMAT(t.early_hiv_date, '%Y') - CAST(t.year_of_birth AS UNSIGNED) as age
        FROM `pqm_vct_recency` as t 
        WHERE 1 = 1
            AND t.site_id IN (@site_id) 
            AND t.early_hiv_date is not null
            AND t.early_hiv_date BETWEEN @from_date AND @to_date
