app.controller('import_read', function ($scope) {
    $scope.init = function () {
        ...
        $(".form select").change(function () {
            ...
            $("#mapCols").val(JSON.stringify(mapCols)).change();
        });

        $("[data-col-index]").each(function () {
            var elm = this;
            var ehead = $(elm).find("[data-col]").attr("data-col");
            if ($(elm).find("select option").filter(function () {
                return $(this).html() == ehead;
            }).length > 0) {
                $(elm).find("select").val($(elm).find("select option").filter(function () {
                    return $(this).html() == ehead;
                }).attr("value")).change();
            } else {
                $(elm).find("select").val($(elm).find("select > option:contains(" + ehead + ")").attr("value")).change();
            }
        });
    };
});

app.controller('import_view', function ($scope) {
...
app.controller('import_upload_pqm', function ($scope, $uibModal, msg) {
    $scope.init = function () {
        $scope.select_search("#sites", "Chọn cơ sở nhập liệu");
        $scope.select_search("#year", "Chọn cơ sở nhập liệu");
        $scope.select_search("#quarter", "Chọn cơ sở nhập liệu");
    };
});
