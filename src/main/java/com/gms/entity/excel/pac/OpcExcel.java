 ...
 * @author pdThang
 */
public class OpcExcel extends BaseView implements IExportExcel {

    private PacOpcForm form;
    private String extension;

    public OpcExcel(PacOpcForm form, String extension) {
        setUseStyle(false);
        this.form = form;
...
//        createFilterRow("Ngày xuất excel", TextUtils.formatDate(new Date(), "dd/MM/yyyy"), cellIndexFilter);
    ...
    private void createTable() throws Exception {
        ...
        headers.add("Trạng thái biến động");
        ...
        if (form.getDataPage() != null && form.getDataPage().getData().size() > 0) {
            List<Object> row;
            int index = 0;
            for (PacPatientInfoEntity item : form.getDataPage().getData()) {
                ...
                row.add(item.getID());
                row.add(StringUtils.isEmpty(item.getOpcCode()) ? "" : item.getOpcCode());
                ...
                row.add(StringUtils.isEmpty(item.getConfirmCode()) ? "" : item.getConfirmCode());
                row.add(item.getConfirmTime() == null ? "" : TextUtils.formatDate(item.getConfirmTime(), "dd/MM/yyyy"));
                row.add(StringUtils.isEmpty(item.getSiteConfirmID()) ? "" : form.getOptions().get(ParameterEntity.PLACE_TEST).getOrDefault(item.getSiteConfirmID(), item.getSiteConfirmID()));
                ...
                row.add(StringUtils.isEmpty(item.getStatusOfChangeTreatmentID()) ? "" : form.getOptions().get(ParameterEntity.STATUS_OF_CHANGE_TREATMENT).get(item.getStatusOfChangeTreatmentID()));

                row.add(form.getHivCodeOptions().getOrDefault(item.getID() + "", ""));

                row.add(form.getOptions().get("provinces").getOrDefault(item.getPermanentProvinceID(), ""));
                row.add(form.getOptions().get("districts").getOrDefault(item.getPermanentDistrictID(), ""));
                row.add(form.getOptions().get("wards").getOrDefault(item.getPermanentWardID(), ""));

                row.add(form.getOptions().get("provinces").getOrDefault(item.getCurrentProvinceID(), ""));
                row.add(form.getOptions().get("districts").getOrDefault(item.getCurrentDistrictID(), ""));
                row.add(form.getOptions().get("wards").getOrDefault(item.getCurrentWardID(), ""));
