 ...
 * @author pdThang
 */
public class AidsExcel extends BaseView implements IExportExcel {

    private PacAidsForm form;
    private String extension;

    public AidsExcel(PacAidsForm form, String extension) {
        this.useStyle = false;
        this.form = form;
...
//        }
    ...
    private void createTable() {
        ...
        //Tiêu đề
        row = createTableHeaderRow("TT", "Đơn vị báo cáo", "Lũy tích", null, null, null, "Còn sống", null, null, null, "Tử vong", null, null, null);
        rownum = row.getRowNum();
        setMerge(rownum, rownum, 2, 5, true);
        setMerge(rownum, rownum, 6, 9, true);
        setMerge(rownum, rownum, 10, 13, true);
        createTableHeaderRow(null, null, "Nam", "Nữ", "Không rõ", "Tổng", "Nam", "Nữ", "Không rõ", "Tổng", "Nam", "Nữ", "Không rõ", "Tổng");
        ...
        if (form.getTable() != null  && form.getSum() > 0) {
            ...
            if (form.isVAAC() && StringUtils.isEmpty(form.getProvince())) {
                ...
                rownum = row.getRowNum();
                setMerge(rownum, rownum, 1, 13, true);
            }
            int stt = 1;

            if (form.getTable() != null) {
                for (TablePacAidsForm genderForm : form.getTable()) {
                    ...
                    if ((StringUtils.isEmpty(form.getLevelDisplay()) || "province".equals(form.getLevelDisplay())) && form.isVAAC()) {
                        createTableRow(stt, genderForm.getDisplayName(), genderForm.getMale(), genderForm.getFemale(), genderForm.getOther(), genderForm.getSum(), 
                                genderForm.getMaleAlive(), genderForm.getFemaleAlive(), genderForm.getOtherAlive(), genderForm.getSumAlive(),
                                 genderForm.getMaleDeath(), genderForm.getFemaleDeath(), genderForm.getOtherDeath(), genderForm.getSumDeath());
                    }

                    // Hiển thị huyện
                    if (genderForm.getChilds() != null && !genderForm.getChilds().isEmpty()) {
                        for (TablePacAidsForm child : genderForm.getChilds()) {
                            ...
                            if ("district".equals(form.getLevelDisplay()) || "other".equals(genderForm.getWardID())) {
                                createTableRow("", "Tỉnh khác".equals(genderForm.getDisplayName()) ? child.getDisplayName() : "     " + child.getDisplayName(), 
                                child.getMale(), child.getFemale(), child.getOther(), child.getSum(), 
                                child.getMaleAlive(), child.getFemaleAlive(), child.getOtherAlive(), child.getSumAlive(),
                                child.getMaleDeath(), child.getFemaleDeath(), child.getOtherDeath(), child.getSumDeath());
                            }

                            // Lặp hiển thị xã
                            if (child.getChilds() != null && !child.getChilds().isEmpty()) {
                                for (TablePacAidsForm childWard : child.getChilds()) {
                                    createTableRow("", "        " + childWard.getDisplayName(), 
                                childWard.getMale(), childWard.getFemale(), childWard.getOther(), childWard.getSum(), 
                                childWard.getMaleAlive(), childWard.getFemaleAlive(), childWard.getOtherAlive(), childWard.getSumAlive(),
                                childWard.getMaleDeath(), childWard.getFemaleDeath(), childWard.getOtherDeath(), childWard.getSumDeath());
                                }
                            }
                            // Tổng huyện
                            if (!"Tỉnh khác".equals(genderForm.getDisplayName()) && "ward".equals(form.getLevelDisplay())
                                ...
                                createTableRow("", "     Tổng huyện", child.getMale(), child.getFemale(), child.getOther(), child.getSum(), 
                                child.getMaleAlive(), child.getFemaleAlive(), child.getOtherAlive(), child.getSumAlive(),
                                child.getMaleDeath(), child.getFemaleDeath(), child.getOtherDeath(), child.getSumDeath());
                            }
                        }
                    ...
                    if (form.getTable().size() > 1 && (!"province".equals(form.getLevelDisplay()) && (StringUtils.isEmpty(form.getProvince()) || form.getProvince().contains(","))) && !"Tỉnh khác".equals(genderForm.getDisplayName())) {
                        createTableRow("", "Tổng tỉnh", genderForm.getMale(), genderForm.getFemale(), genderForm.getOther(), genderForm.getSum(), 
                                genderForm.getMaleAlive(), genderForm.getFemaleAlive(), genderForm.getOtherAlive(), genderForm.getSumAlive(),
                                genderForm.getMaleDeath(), genderForm.getFemaleDeath(), genderForm.getOtherDeath(), genderForm.getSumDeath());
                    }
                    stt++;
                ...
                // Dòng tổng cộng
                row = createTableRow("Tổng cộng", null, form.getMale(), form.getFemale(), form.getOther(), form.getSum(), 
                                form.getMaleAlive(), form.getFemaleAlive(), form.getOtherAlive(), form.getSumAlive(),
                                form.getMaleDeath(), form.getFemaleDeath(), form.getOtherDeath(), form.getSumDeath());
                rownum = row.getRowNum();
