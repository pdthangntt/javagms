 ...
 * @author pdThang
 */
public class PatientExportAIDSExcel extends BaseView implements IExportExcel {

    private PacNewForm form;
    private String extension;

    public PatientExportAIDSExcel(PacNewForm form, String extension) {
        setUseStyle(false);
        this.form = form;
    ...
    private void createTable() {
        ...
        cell = createCell(row, indexT++);
        cell.setCellValue("NoiLayMau2");
        cell.setCellStyle(th);

        cell = createCell(row, indexT++);
        cell.setCellValue("IDNoiLayMau3");
        cell.setCellStyle(th);
        ...
        if (form.getItems().size() > 0) {
            ...
            for (PacPatientInfoEntity item : form.getItems()) {
                ...
                cell = createCell(row, index++);
                cell.setCellValue(item.getRequestDeathTime()== null ? "" : TextUtils.formatDate(item.getRequestDeathTime(), "dd/MM/yyyy"));
                ...
                cell = createCell(row, index++);
                cell.setCellValue(item.getInsuranceExpiry() == null ? "" : TextUtils.formatDate(item.getInsuranceExpiry(), "dd/MM/yyyy"));
                ...
                cell = createCell(row, index++);
                cell.setCellValue(item.getRegistrationType());

                cell = createCell(row, index++);
                cell.setCellValue(item.getFirstTreatmentTime() == null ? "" : TextUtils.formatDate(item.getFirstTreatmentTime(), "dd/MM/yyyy"));
                ...
                cell = createCell(row, index++);
                cell.setCellValue(item.getEndTime() == null ? "" : TextUtils.formatDate(item.getEndTime(), "dd/MM/yyyy"));
                ...
                cell = createCell(row, index++);
                cell.setCellValue(item.getOpcCode());
            }
        }
    }
