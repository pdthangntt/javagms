 ...
 * @author TrangBN
 */
public class DetectHivInsuranceExcel extends BaseView implements IExportExcel {

    private DetectHivInsuranceForm form;
    private String extension;

    public DetectHivInsuranceExcel(DetectHivInsuranceForm form, String extension) {
        setUseStyle(false);
        this.form = form;
        this.extension = extension;
        this.sheetName = "INSURANCE";
    }
    ...
    private void createTable() {
        ...
        //Tiêu đề
        row = createTableHeaderRow("TT", "Đơn vị báo cáo", "Có BHYT", null, "Không có BHYT", null, "Không có thông tin", null, "Tổng", null);
        ...
        if (form.getTable() != null  && form.getSum() > 0) {
            ...
            if (form.getTable() != null) {
                for (TableDetectHivInsuranceForm genderForm : form.getTable()) {

                    // Cộng tổng cộng
                    maleTotal += genderForm.getYes();
                    femaleTotal += genderForm.getNo();
                    ...
                    if ((StringUtils.isEmpty(form.getLevelDisplay()) || "province".equals(form.getLevelDisplay())) && form.isVAAC()) {
                        createTableRow(stt, genderForm.getDisplayName(), genderForm.getYes(), genderForm.getYesPercent(),
                                genderForm.getNo(), genderForm.getNoPercent(), genderForm.getOther(), genderForm.getOtherPercent(),
                                genderForm.getSum(), genderForm.getSumPercent(form.getSum()));
                    }

                    // Hiển thị huyện
                    if (genderForm.getChilds() != null && !genderForm.getChilds().isEmpty()) {
                        for (TableDetectHivInsuranceForm child : genderForm.getChilds()) {
                            ...
                            if ("district".equals(form.getLevelDisplay()) || "other".equals(genderForm.getWardID())) {
                                ...
                                createTableRow("", displayName, child.getYes(), child.getYesPercent(),
                                        child.getNo(), child.getNoPercent(), child.getOther(), child.getOtherPercent(),
                                        child.getSum(), child.getSumPercent(form.getSum()));
                            }

                            // Lặp hiển thị xã
                            if (child.getChilds() != null && !child.getChilds().isEmpty()) {
                                for (TableDetectHivInsuranceForm childWard : child.getChilds()) {
                                    createTableRow("", "        " + childWard.getDisplayName(), childWard.getYes(), childWard.getYesPercent(),
                                            childWard.getNo(), childWard.getNoPercent(), childWard.getOther(), childWard.getOtherPercent(),
                            ...
                            if (!"Tỉnh khác".equals(genderForm.getDisplayName()) && "ward".equals(form.getLevelDisplay())
                                ...
                                createTableRow("", "     Tổng huyện", child.getYes(), child.getYesPercent(),
                                        child.getNo(), child.getNoPercent(), child.getOther(), child.getOtherPercent(),
                                        child.getSum(), child.getSumPercent(form.getSum()));
                            }
                        }
                    ...
                    if (form.getTable().size() > 1 && (!"province".equals(form.getLevelDisplay()) && (StringUtils.isEmpty(form.getProvince()) || form.getProvince().contains(","))) && !"Tỉnh khác".equals(genderForm.getDisplayName())) {
                        createTableRow("", "Tổng tỉnh", genderForm.getYes(), genderForm.getYesPercent(),
                                genderForm.getNo(), genderForm.getNoPercent(), genderForm.getOther(), genderForm.getOtherPercent(),
                                genderForm.getSum(), genderForm.getSumPercent(form.getSum()));
                    }
                    stt++;
