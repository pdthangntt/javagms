 ...
 * @author pdThang
 */
public class PacOutProvinceResidentForm extends BaseForm {
    ...
    private Map<String, HashMap<String, PacOutProvinceResidentTable>> items;
    private PacOutProvinceResidentTable table;
    
    public PacOutProvinceResidentTable getTotal(Map<String, HashMap<String, PacOutProvinceResidentTable>> item1, String key) {
        PacOutProvinceResidentTable model = new PacOutProvinceResidentTable();

        for (Map.Entry<String, HashMap<String, PacOutProvinceResidentTable>> entry : item1.entrySet()) {
            String key1 = entry.getKey();
            HashMap<String, PacOutProvinceResidentTable> value = entry.getValue();

            model.setDangODiaPhuong(model.getDangODiaPhuong() + value.get(key).getDangODiaPhuong());
                model.setDiLamAnXa(model.getDiLamAnXa() + value.get(key).getDiLamAnXa());
                model.setDiTrai(model.getDiTrai() + value.get(key).getDiTrai());
                model.setChuyenDiNoiKhac(model.getChuyenDiNoiKhac() + value.get(key).getChuyenDiNoiKhac());
                model.setChuyenTrongTinh(model.getChuyenTrongTinh() + value.get(key).getChuyenTrongTinh());
                model.setMatDau(model.getMatDau() + value.get(key).getMatDau());
                model.setChuaXacDinhDuoc(model.getChuaXacDinhDuoc() + value.get(key).getChuaXacDinhDuoc());
                model.setKhongCoThucTe(model.getKhongCoThucTe() + value.get(key).getKhongCoThucTe());
                model.setUnclear(model.getUnclear() + value.get(key).getUnclear());
    ...
    public Map<String, HashMap<String, PacOutProvinceResidentTable>> getItems() {
        return items;
    }

    public void setItems(Map<String, HashMap<String, PacOutProvinceResidentTable>> items) {
        this.items = items;
    }

    public PacOutProvinceResidentTable getTable() {
        return table;
    }

    public void setTable(PacOutProvinceResidentTable table) {
        this.table = table;
    }
