 ...
 * @author pdThang
 */
public class PacOutProvinceTransmissionForm extends BaseForm {
    ...
    private Map<String, HashMap<String, PacOutProvinceTransmissionTable>> items;
    private PacOutProvinceTransmissionTable table;

    public PacOutProvinceTransmissionTable getTotal(Map<String, HashMap<String, PacOutProvinceTransmissionTable>> item1, String key) {
        PacOutProvinceTransmissionTable model = new PacOutProvinceTransmissionTable();

        for (Map.Entry<String, HashMap<String, PacOutProvinceTransmissionTable>> entry : item1.entrySet()) {
            String key1 = entry.getKey();
            HashMap<String, PacOutProvinceTransmissionTable> value = entry.getValue();

            model.setBlood(model.getBlood() + value.get(key).getBlood());
            model.setSexuality(model.getSexuality() + value.get(key).getSexuality());
            model.setMomtochild(model.getMomtochild() + value.get(key).getMomtochild());
            model.setUnclear(model.getUnclear() + value.get(key).getUnclear());
            model.setNoinformation(model.getNoinformation() + value.get(key).getNoinformation());
        }
        return model;
    }
    ...
    public Map<String, HashMap<String, PacOutProvinceTransmissionTable>> getItems() {
        return items;
    }

    public void setItems(Map<String, HashMap<String, PacOutProvinceTransmissionTable>> items) {
        this.items = items;
    }

    public PacOutProvinceTransmissionTable getTable() {
        return table;
    }

    public void setTable(PacOutProvinceTransmissionTable table) {
        this.table = table;
    }
