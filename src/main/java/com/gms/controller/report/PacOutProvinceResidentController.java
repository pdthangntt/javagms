...
import com.gms.entity.excel.pac.OutProvinceResidentExcel;
 ...
 * @author pdThang
 */
@Controller(value = "pacOutProvinceResident")
public class PacOutProvinceResidentController extends BaseController {
    ...
    private PacOutProvinceResidentForm getData(
        ...
        PacOutProvinceResidentForm form = new PacOutProvinceResidentForm();
        form.setFileName("BaoCaoNgoaiTinhTheoHienTrangCuTru_" + TextUtils.formatDate(new Date(), "hhmm_ddMMyyyy"));
        form.setTitle("Báo cáo người nhiễm ngoại tỉnh theo hiện trạng cư trú");
        ...
        form.setItems(pacPatientRepositoryImpl.getDataOutprovinceResident(provinceIDs,
        ...
        PacOutProvinceResidentTable table = new PacOutProvinceResidentTable();
        for (Map.Entry<String, HashMap<String, PacOutProvinceResidentTable>> entry : form.getItems().entrySet()) {
            ...
            for (Map.Entry<String, PacOutProvinceResidentTable> entry1 : value.entrySet()) {
                PacOutProvinceResidentTable value1 = entry1.getValue();
                table.setDangODiaPhuong(table.getDangODiaPhuong() + value1.getDangODiaPhuong());
                table.setDiLamAnXa(table.getDiLamAnXa() + value1.getDiLamAnXa());
                table.setDiTrai(table.getDiTrai() + value1.getDiTrai());
                table.setChuyenDiNoiKhac(table.getChuyenDiNoiKhac() + value1.getChuyenDiNoiKhac());
                table.setChuyenTrongTinh(table.getChuyenTrongTinh() + value1.getChuyenTrongTinh());
                table.setMatDau(table.getMatDau() + value1.getMatDau());
                table.setChuaXacDinhDuoc(table.getChuaXacDinhDuoc() + value1.getChuaXacDinhDuoc());
                table.setKhongCoThucTe(table.getKhongCoThucTe() + value1.getKhongCoThucTe());
                table.setUnclear(table.getUnclear() + value1.getUnclear());
    ...
    @GetMapping(value = {"/pac-out-province-resident/index.html"})
    public String actionIndex(Model model,
        ...
        PacOutProvinceResidentForm form = getData(updateTimeFrom, updateTimeTo,
                patientStatus,
        ...
        return "report/pac/out-province-resident";
    }

    @GetMapping(value = {"/pac-out-province-resident/excel.html"})
    public ResponseEntity<InputStreamResource> actionPositiveExcel(
        ...
        return exportExcel(new OutProvinceResidentExcel(form, EXTENSION_EXCEL_X));
    }

    @GetMapping(value = {"/pac-out-province-resident/mail.html"})
    public String actionSendEmail(Model model, RedirectAttributes redirectAttributes,
        ...
        if (currentUser.getUser().getEmail() == null || "".equals(currentUser.getUser().getEmail())) {
            ...
            return redirect(UrlUtils.pacOutProvinceResident("index"));
        }
        PacOutProvinceResidentForm form = getData(updateTimeFrom, updateTimeTo,
                patientStatus,
                manageTimeFrom, manageTimeTo,
        ...
        sendEmailAtachmentExcel(
                ...
                context,
                new OutProvinceResidentExcel(form, EXTENSION_EXCEL_X));
    ...
    @GetMapping(value = {"/pac-out-province-resident/pdf.html"})
    public ResponseEntity<InputStreamResource> actionPdf(
        ...
        return exportPdf(form.getFileName(), "report/pac/out-province-resident-pdf.html", context);
    }
