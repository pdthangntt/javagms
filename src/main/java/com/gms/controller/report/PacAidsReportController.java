...
import com.gms.entity.excel.pac.AidsReportExcel;
 ...
 * @author DSNAnh
 */
@Controller(value = "report_pac_aids_report")
public class PacAidsReportController extends BaseController {
    protected static final String PDF_FILE_NAME = "DS benh nhan AIDS_";
    ...
    @GetMapping(value = {"/pac-aids-report/index.html"})
    public String actionIndex(Model model,
        ...
        PacAidsReportForm form = getData(aidsFrom, aidsTo, code, name, blood, updateTimeFrom, updateTimeTo, addressFilter, 
                                    confirmTimeFrom, confirmTimeTo, inputTimeFrom, inputTimeTo, deathTimeFrom, deathTimeTo, provinceID, districtID, 
                                    wardID, testObject, race, gender, transmision, ageFrom, ageTo, manageStt, placeTest, facility, page, size);

        DataPage<PacPatientInfoEntity> dataPage = pacPatientService.findAidsReport(form.getSearch());
        ...
        LinkedHashMap<String, String> manageStatus = new LinkedHashMap<>();
        ...
        model.addAttribute("title", "Danh sách bệnh nhân AIDS");
        ...
        return "report/pac/pac-aids-report";
    }

    @GetMapping(value = {"/pac-aids-report/email.html"})
    public String actionSendEmail(Model model,
            ...
            @RequestParam(name = "age_from", required = false) String ageFrom,
            @RequestParam(name = "age_to", required = false) String ageTo,
            @RequestParam(name = "manage_status", required = false, defaultValue = "") String manageStt,
            @RequestParam(name = "code", required = false) String code,
            @RequestParam(name = "name", required = false) String name,
            @RequestParam(name = "blood", required = false) String blood,
            @RequestParam(name = "place_test", required = false) String placeTest,  // Nơi xét nghiệm khẳng định
        ...
        if (StringUtils.isEmpty(currentUser.getUser().getEmail())) {
            ...
            return redirect(UrlUtils.pacAidsReport());
        }
        PacAidsReportForm form = getData(aidsFrom, aidsTo,code, name, blood, updateTimeFrom, updateTimeTo, addressFilter, 
                                    confirmTimeFrom, confirmTimeTo, inputTimeFrom, inputTimeTo, deathTimeFrom, deathTimeTo, provinceID, districtID, 
                                    wardID, testObject, race, gender, transmision, ageFrom, ageTo, manageStt, placeTest, facility, page, size);
        ...
        sendEmailAtachmentExcel(
                ...
                "Danh sách bệnh nhân AIDS",
                EmailoutboxEntity.TEMPLATE_PAC_AIDS_REPORT,
                context,
                new AidsReportExcel(form, EXTENSION_EXCEL_X));
    ...
    @GetMapping(value = {"/pac-aids-report/excel.html"})
    public ResponseEntity<InputStreamResource> actionExportExcel(
            ...
            @RequestParam(name = "age_from", required = false) String ageFrom,
            @RequestParam(name = "age_to", required = false) String ageTo,
            @RequestParam(name = "manage_status", required = false) String manageStt,
            @RequestParam(name = "code", required = false) String code,
            @RequestParam(name = "name", required = false) String name,
            @RequestParam(name = "blood", required = false) String blood,
            @RequestParam(name = "place_test", required = false) String placeTest,  // Nơi xét nghiệm khẳng định
        ...
        form.setAllProvincesName(allProvinces);
        form.setItems(dataPage.getData());
        return exportExcel(new AidsReportExcel(form, EXTENSION_EXCEL_X));
    }

    @GetMapping(value = {"/pac-aids-report/pdf.html"})
    public ResponseEntity<InputStreamResource> actionIn(Model model,
            ...
            @RequestParam(name = "page", required = false, defaultValue = "1") int page,
            @RequestParam(name = "size", required = false, defaultValue = "999999") int size) throws Exception {
        PacAidsReportForm form = getData(aidsFrom, aidsTo,code, name, blood, updateTimeFrom, updateTimeTo, addressFilter, confirmTimeFrom, 
                                    confirmTimeTo, inputTimeFrom, inputTimeTo, deathTimeFrom, deathTimeTo, provinceID, districtID, wardID, testObject, race, 
                                    gender, transmision, ageFrom, ageTo, manageStt, placeTest, facility, page, size);
        DataPage<PacPatientInfoEntity> dataPage = pacPatientService.findAidsReport(form.getSearch());
        HashMap<String, Object> context = new HashMap<>();
        ...
        return exportPdf(PDF_FILE_NAME + TextUtils.removeDiacritical(getCurrentUser().getSite().getShortName()), "report/pac/pac-aids-report-pdf.html", context);
    }
    ...
    private PacAidsReportForm getData(String aidsFrom, String aidsTo, String code, String name, String blood, String updateTimeFrom, String updateTimeTo, String addressFilter, String confirmTimeFrom, String confirmTimeTo, String inputTimeFrom, String inputTimeTo,
            String deathTimeFrom, String deathTimeTo, String provinceID, String districtID, String wardID, String testObject,
            String race, String gender, String transmision, String ageStart, String ageEnd, String managestatus, String placeTest, String facility, int page, int size) throws ParseException {
        String ageFrom = "";
        String ageTo = "";
        ...
        if (StringUtils.isEmpty(provinceID) && StringUtils.isEmpty(districtID) && StringUtils.isEmpty(wardID)) {
            if (isTYT()) {
        ...
        PacAidsReportForm form = new PacAidsReportForm();

        form.setTitle("DANH SÁCH BỆNH NHÂN AIDS");
        if (!isVAAC()) {
            switch (managestatus) {
                case "-1":
                    form.setTitle("Danh sách bệnh nhân AIDS ");
                    break;
                case "1":
                    form.setTitle("Danh sách bệnh nhân AIDS chưa rà soát ");
                    break;
                case "2":
                    form.setTitle("Danh sách bệnh nhân AIDS cần rà soát ");
                    break;
                case "3":
                    form.setTitle("Danh sách bệnh nhân AIDS đã rà soát ");
                    break;
                case "4":
                    form.setTitle("Danh sách bệnh nhân AIDS được quản lý ");
                    break;
                default:
                    form.setTitle("");
            }
        }
        form.setFileName("DanhSachBenhNhanAIDS_" + "_" + TextUtils.formatDate(new Date(), "hhmm_ddMMyyyy"));
        ...
        PacAidsReportSearch search = new PacAidsReportSearch();
        ...
        search.setIsTYT(isTYT());
        search.setIsTTYT(isTTYT());
        ...
        form.setTitleLocationDisplay(getTitleAddress(pIDsDisplay, dIDsDisplay, wIDsDisplay));
        form.setProvinceName(StringUtils.isEmpty(getTitleAddress(pIDs, dIDs, wIDs)) && isVAAC() ? "Quốc gia" : currentUser.getSiteProvince().getName());
